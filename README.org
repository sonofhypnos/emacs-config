...
Largest influences on this configuration have been:
...
Set up multiple languages for spell-checking. This was largly stolen from [[https://github.com/munen/emacs.d ][this]] repo.
#+begin_src emacs-lisp
(after! ispell
...
(defun pre-tangle-config ()
  (and (file-in-directory-p
       ...
       (shell-command "cp config.org README.org && sed -i '/^[^\"]*TODO[^\"]*$/d' README.org; sed -i '/^[^\"]*DONE[^\"]*$/d' config.md; doom compile")))
