...
(defun tassilo/org-capture-cleanup ()
  ...
  (and (tassilo/scratch-window-p)
       ;for some weird reason this worked better for me than just (delete-frame), so as long as it works I won't touch it.
      (progn
        (progn
...
(defun tassilo/org-capture-setup ()
  (and (tassilo/scratch-window-p)
       (message "tassilo/org-capture-setup was called by org-capture hook as expected")
       (delete-other-windows))) ;no idea why this is not working.
(add-hook 'org-capture-mode-hook #'tassilo/org-capture-setup)
#+end_src


*** Setting up org-capture
Trying to set up org-capture has been a nightmare for me so far. It is broken again at the moment and I haven't found the time to fix it. It took me quite some time to finally get org-capture to behave nicely with my browser (Firefox) on Ubuntu.
- Update 08-09-2021: org-protocol is broken again and I haven't had the time to fix it yet.
...
(defun tassilo/post-tangle-config ()
    (and (file-in-directory-p
       ...
       (async-shell-command "yes | doom compile & cp config.org README.org && sed -i '/^[^\"]*TODO[^\"]*$/d' README.org" "*config-update*")))
(add-to-list 'display-buffer-alist '("*config-update*" display-buffer-no-window (nil)))
...
#+begin_src emacs-lisp
(after! emr (define-key prog-mode-map (kbd "M-RET") 'emr-show-refactor-menu))
#+end_src

See https://opensource.com/article/20/3/blog-emacs
#+begin_src emacs-lisp
